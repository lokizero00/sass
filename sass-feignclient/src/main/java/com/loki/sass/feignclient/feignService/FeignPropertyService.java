...
public interface FeignPropertyService {
    ...
    ResultDTO<PageInfo<PropertyDTO>> getPropertyListSearch(@RequestParam("propertyQueryJson") String propertyQueryJson) throws BizException;

    @RequestMapping(method = RequestMethod.POST,value = "/property/v1/addProperty")
    ResultDTO<Boolean> addProperty(@RequestParam("propertyRequestStr") String propertyRequestStr) throws BizException;

    @RequestMapping(method = RequestMethod.POST,value = "/property/v1/editProperty")
    ResultDTO<Boolean> editProperty(@RequestParam("propertyRequestStr") String propertyRequestStr) throws BizException;

    @RequestMapping(method = RequestMethod.POST,value = "/property/v1/deleteProperty")
    ResultDTO<Boolean> deleteProperty(@RequestParam("propertyId") Integer propertyId, @RequestParam("adminId") Integer adminId) throws BizException;
}
